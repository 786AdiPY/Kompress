...
def get_repo_root():
    ...
    while os.path.dirname(cur) != cur:
        if os.path.exists(os.path.join(cur, "pyproject.toml")) or os.path.exists(os.path.join(cur, "pipeline.yaml")):
            return cur
...
from kompress.engine.common.hardware import known_targets                 # noqa: E402
from kompress.engine.registry import mlflow_state, promote                 # noqa: E402
from kompress.engine.export import get_exporter, list_exporters            # noqa: E402
from kompress.services.cloud.worker.executor import (                      # noqa: E402
    TRACKING_URI, EXPERIMENT, RUNS_DIR, PointerError,
    check_ref_policy, execute_job, registered_name,
)
from kompress.services.cloud.worker.queue import get_queue                          # noqa: E402
from kompress.engine.common.limits import MAX_UPLOAD_BYTES, human_size       # noqa: E402
...
class _SizeLimitedReader:
    """Wraps an uploaded file's underlying stream, aborting once more than `limit`
    bytes have been read. boto3's upload_fileobj reads via .read(n) in chunks, so
    this catches an oversized upload mid-stream — not just when Content-Length lied
    or was absent (chunked transfer) — before the whole file is buffered/stored."""
    ...
    def read(self, n: int = -1) -> bytes:
        ...
        if self._seen > self._limit:
            raise _UploadTooLarge(self._seen)
        return chunk


@app.post("/uploads", status_code=201)
async def upload_file(request: Request, file: UploadFile = File(...), kind: str = "model"):
    """Accept a file upload (a user's .pkl model or test CSV) UNDER the direct-upload
    size ceiling (MAX_UPLOAD_BYTES, default 10GB), stream it into object storage, and
    return an s3:// pointer to hand to POST /runs. This is how the UI can offer
    'upload a file' while the engine still only ever consumes pointers — the raw bytes
    never reach the compression process, only the resulting pointer does.

    Models at/above the ceiling are out of scope for this endpoint by design: host the
    file yourself and submit its s3://... pointer directly to POST /runs instead."""
    import uuid
    from kompress.engine.common import objstore

    # Fast pre-check when the client declares Content-Length — avoids opening a
    # connection to object storage at all for an obviously oversized file.
    declared = request.headers.get("content-length")
    if declared and declared.isdigit() and int(declared) > MAX_UPLOAD_BYTES:
        raise HTTPException(
            413, f"File is {human_size(int(declared))}, over the "
                 f"{human_size(MAX_UPLOAD_BYTES)} direct-upload limit. For models this "
                 f"size, host it yourself and submit its s3://... pointer to POST /runs.")

    safe_name = os.path.basename(file.filename or "upload.bin")
    key = f"uploads/{kind}/{uuid.uuid4().hex}_{safe_name}"
    try:
        ref = objstore.upload_fileobj(_SizeLimitedReader(file.file, MAX_UPLOAD_BYTES), key)
    except _UploadTooLarge as e:
        raise HTTPException(
            413, f"File exceeds the {human_size(MAX_UPLOAD_BYTES)} direct-upload limit "
                 f"(stopped after {human_size(e.seen)}). For models this size, host it "
                 f"yourself and submit its s3://... pointer to POST /runs instead.")
