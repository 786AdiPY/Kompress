...
from kompress.engine.common.hardware import compressors_for, known_targets  # noqa: E402
...
def main():
    ...
    for script in ("src/kompress/engine/pipeline/compress.py", "src/kompress/engine/pipeline/benchmark.py"):
        if run_stage(script, env) != 0:
            print(f"\nStage {script} failed — aborting job.")
            sys.exit(2)

    gate_rc = run_stage("src/kompress/engine/pipeline/gate.py", env)          # non-zero if gate fails
    run_stage("src/kompress/engine/pipeline/generate_report.py", env)        # always emit the Plan
